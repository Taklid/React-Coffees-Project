...
import neutration from '../assets/nutrition.png'
...
const CoffeeDtls = () => {
    ...
    useEffect(() => {
        const singelData = data.find(coffee => coffee.id == id)
        setCoffee(singelData)
        const favorites =  getAllFavorite()
        const isExist = favorites.find(item => item.id == singelData.id)
        if (isExist) {
            setIsFavorite(true)
        }
        ...
        <div>
           <div className='my-12 px-4'>
            ...
            <div className='w-full h-full md:h-[500px] object-cover overflow-hidden rounded-xl shadow-xl'>
                <img className='w-[800px] lg:h-[500px] h-[350px] mx-auto rounded-xl' src={image} alt='' />
            </div>
